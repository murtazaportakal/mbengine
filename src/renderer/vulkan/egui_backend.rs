...
pub struct EguiBackend {
    ...
    pub user_textures: std::collections::HashMap<egui::TextureId, vk::DescriptorSet>,
    next_user_texture_id: u64,
}

impl EguiBackend {
    pub fn new(vulkan: &VulkanDevice, color_format: vk::Format) -> Self {
        unsafe {
    ...
    pub fn free_texture(&mut self, _vulkan: &VulkanDevice, id: egui::TextureId) {
    ...
    pub fn register_user_texture(
        ...
        unsafe {
            ...
            self.user_textures.insert(tex_id, descriptor_set);
    ...
    pub fn update_user_texture(
        ...
        if let egui::TextureId::User(_) = texture_id {
            if let Some(&descriptor_set) = self.user_textures.get(&texture_id) {
                unsafe {
                    let image_info = vk::DescriptorImageInfo::default()
                        .image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL)
                        .image_view(view)
    ...
    pub fn draw(
        ...
        unsafe {
            let vb_ptr = vulkan
                .device
                .map_memory(vb.memory, 0, vk::WHOLE_SIZE, vk::MemoryMapFlags::empty())
            ...
            vulkan.device.cmd_push_constants(
                ...
                std::slice::from_raw_parts(
                    &screen_size_in_points as *const _ as *const u8,
            ...
            let viewport = vk::Viewport {
                x: 0.0,
                y: 0.0,
                width: screen_size_in_points[0] * pixels_per_point,
                height: screen_size_in_points[1] * pixels_per_point,
                min_depth: 0.0,
                max_depth: 1.0,
            };
            vulkan
                ...
                if let egui::epaint::Primitive::Mesh(mesh) = &p.primitive {
                    if mesh.texture_id != current_texture_id {
                        let descriptor_set = *self
                            .user_textures
                            .get(&mesh.texture_id)
                            .unwrap_or(&self.font_descriptor_set);
                        vulkan.device.cmd_bind_descriptor_sets(
                            command_buffer,
                            vk::PipelineBindPoint::GRAPHICS,
                            self.pipeline_layout,
